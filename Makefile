...
# may rely on it.
#LM include Makefile.legacy

include package/Makefile.in
include support/dependencies/dependencies.mk

#LM include toolchain/*.mk
#LM include toolchain/*/*.mk
...
#LM include boot/common.mk
#LM include linux/linux.mk
#LM include fs/common.mk
...
#LM	@$(call MESSAGE,"Finalizing target directory")
#LM	$(foreach hook,$(TARGET_FINALIZE_HOOKS),$($(hook))$(sep))
#LM	rm -rf $(TARGET_DIR)/usr/include $(TARGET_DIR)/usr/share/aclocal \
#LM		$(TARGET_DIR)/usr/lib/pkgconfig $(TARGET_DIR)/usr/share/pkgconfig \
#LM		$(TARGET_DIR)/usr/lib/cmake $(TARGET_DIR)/usr/share/cmake
#LM	find $(TARGET_DIR)/usr/{lib,share}/ -name '*.cmake' -print0 | xargs -0 rm -f
#LM	find $(TARGET_DIR)/lib/ $(TARGET_DIR)/usr/lib/ $(TARGET_DIR)/usr/libexec/ \
#LM		\( -name '*.a' -o -name '*.la' \) -print0 | xargs -0 rm -f
#LM ifneq ($(BR2_PACKAGE_GDB),y)
#LM	rm -rf $(TARGET_DIR)/usr/share/gdb
#LM endif
#LM ifneq ($(BR2_PACKAGE_BASH),y)
#LM	rm -rf $(TARGET_DIR)/usr/share/bash-completion
#LM endif
#LM ifneq ($(BR2_PACKAGE_ZSH),y)
#LM 	rm -rf $(TARGET_DIR)/usr/share/zsh
#LM endif
#LM 	rm -rf $(TARGET_DIR)/usr/man $(TARGET_DIR)/usr/share/man
#LM 	rm -rf $(TARGET_DIR)/usr/info $(TARGET_DIR)/usr/share/info
#LM 	rm -rf $(TARGET_DIR)/usr/doc $(TARGET_DIR)/usr/share/doc
#LM 	rm -rf $(TARGET_DIR)/usr/share/gtk-doc
#LM 	-rmdir $(TARGET_DIR)/usr/share 2>/dev/null
#LM 	$(STRIP_FIND_CMD) | xargs -0 $(STRIPCMD) 2>/dev/null || true
#LM 
# See http://sourceware.org/gdb/wiki/FAQ, "GDB does not see any threads
# besides the one in which crash occurred; or SIGTRAP kills my program when
# I set a breakpoint"
#LM ifeq ($(BR2_TOOLCHAIN_HAS_THREADS),y)
#LM 	find $(TARGET_DIR)/lib/ -type f -name 'libpthread*.so*' | \
#LM 		xargs -r $(STRIPCMD) $(STRIP_STRIP_DEBUG)
#LM endif
#LM 
# Valgrind needs ld.so with enough information, so only strip
# debugging symbols.
#LM 	find $(TARGET_DIR)/lib/ -type f -name 'ld-*.so*' | \
#LM 		xargs -r $(STRIPCMD) $(STRIP_STRIP_DEBUG)
#LM 	test -f $(TARGET_DIR)/etc/ld.so.conf && \
#LM 		{ echo "ERROR: we shouldn't have a /etc/ld.so.conf file"; exit 1; } || true
#LM 	test -d $(TARGET_DIR)/etc/ld.so.conf.d && \
#LM 		{ echo "ERROR: we shouldn't have a /etc/ld.so.conf.d directory"; exit 1; } || true
#LM 	mkdir -p $(TARGET_DIR)/etc
#LM 	( \
#LM 		echo "NAME=Buildroot"; \
#LM 		echo "VERSION=$(BR2_VERSION_FULL)"; \
#LM 		echo "ID=buildroot"; \
#LM 		echo "VERSION_ID=$(BR2_VERSION)"; \
#LM 		echo "PRETTY_NAME=\"Buildroot $(BR2_VERSION)\"" \
#LM 	) >  $(TARGET_DIR)/etc/os-release
